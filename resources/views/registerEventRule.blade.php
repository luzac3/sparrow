<!DOCTYPE html>
<html>
<head>

@include('share.htmlHeader)

<link rel="stylesheet" href="/sparrow/css/login.css" type="text/css" media="screen">
<script src="/sparrow/scripts/login.js"></script>
<title>雀オフ　イベント登録(ルール)</title>
</head>
<body>
<form class="form-signin">
  <h1 class="h3 mb-3 font-weight-normal">メンバー登録</h1>
  <p>
    <!-- この部分もフラグとして保持 -->
    <input type="raio" name="registerationMethod" value="" selected="selected">簡易
    <input type="raio" name="registerationMethod" value="">詳細
  </p>
  <div class="easy">
    <table>
      <tr>
        <th>喰いタン</th>
        <td><input type="radio" name="kuitan" value="">あり</td>
        <td><input type="radio" name="kuitan" value="">なし</td>
      </tr>
    </table>
    <p>

    </p>
  </div>
  <div class="detail">
    <table>
      <thead>
        <tr rowspan="3">
          一般役
        </tr>
      </thead>
      <tbody>
        <tr>
          <th>喰いタン</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>後付け</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>ピンヅモ</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>一発</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>流し満貫</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>国士無双時暗槓への槍槓</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>發なし緑一色</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>役満の複合</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <!-- 複合がアリの場合のみ -->
        <tr>
          <th>いくつまで複合するか</th>
          <td rowspan="2">
            <select class="" name="">
              <option value="0">無制限</option>
              <option value="2">2:ダブル役満</option>
              <option value="3">3:トリプル役満</option>
              <option value="4">4:クオドラプル役満</option>
              <option value="5">5:クインティプル役満</option>
              <option value="6">6:セクスタプル役満</option>
              <option value="7">7:セプタプル役満</option>
            </select>
          </td>
        </tr>
        <tr>
          <th rowspan="2">単独役でのダブル役満</th>
        </tr>
        <tr>
          <th>四暗刻単騎</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>国士無双13面待ち</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>純正九蓮宝塔</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>發なし緑一色</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>大四喜</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>四槓子</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
      </tbody>
      <thead>
        <tr rowspan="3">
          流局
        </tr>
      </thead>
      <tbody>
        <tr>
          <th>四風連打</th>
          <td><input type="radio" name="kuitan" value="">流局</td>
          <td><input type="radio" name="kuitan" value="">続行</td>
        </tr>
        <tr>
          <th>九種九牌</th>
          <td><input type="radio" name="kuitan" value="">流局</td>
          <td><input type="radio" name="kuitan" value="">続行</td>
        </tr>
        <tr>
          <th>三家和</th>
          <td><input type="radio" name="kuitan" value="">流局</td>
          <td><input type="radio" name="kuitan" value="">続行</td>
        </tr>
        <tr>
          <th>四家和</th>
          <td><input type="radio" name="kuitan" value="">流局</td>
          <td><input type="radio" name="kuitan" value="">続行</td>
        </tr>
        <tr>
          <th>四開槓</th>
          <td><input type="radio" name="kuitan" value="">流局</td>
          <td><input type="radio" name="kuitan" value="">続行</td>
        </tr>
      </tbody>
      <thead>
        <tr rowspan="3">
          王牌
        </tr>
      </thead>
      <tbody>
        <tr>
          <th>裏ドラ</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>槓ドラ(暗槓)</th>
          <td><input type="radio" name="kuitan" value="">即めくり</td>
          <td><input type="radio" name="kuitan" value="">後めくり</td>
        </tr>
        <tr>
          <th>槓ドラ(明槓)</th>
          <td><input type="radio" name="kuitan" value="">即めくり</td>
          <td><input type="radio" name="kuitan" value="">後めくり</td>
        </tr>
      </tbody>
      <thead>
        <tr rowspan="3">
          包
        </tr>
      </thead>
      <tbody>
        <tr>
          <th>大三元</th>
          <td><input type="radio" name="kuitan" value="">責任払い</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>四喜和</th>
          <td><input type="radio" name="kuitan" value="">責任払い</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>大明槓嶺上開花</th>
          <td><input type="radio" name="kuitan" value="">責任払い</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
      </tbody>
      <thead>
        <tr rowspan="3">
          特殊ルール
        </tr>
      </thead>
      <tbody>
        <tr>
          <th>永田町ルール</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>割れ目</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>青天井</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>同時和了</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <!-- 同時和了がアリの場合のみ -->
        <tr>
          <th>同時に和了できる人数</th>
          <td rowspan="2">
            <select class="" name="">
              <option value="2">2人</option>
              <option value="3">3人</option>
            </select>
          </td>
        </tr>
      </tbody>
      <thead>
        <tr rowspan="3">
          ローカル役1
        </tr>
      </thead>
      <tbody>
        <tr>
          <th>オープンリーチ</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>三連刻</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>一色三順</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>パンダ</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>大車輪</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>四連刻</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>八連荘</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>人和</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
      </tbody>
      <thead>
        <tr rowspan="3">
          ローカル役2
        </tr>
      </thead>
      <tbody>
        <tr>
          <th>オープンリーチ</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>三連刻</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>一色三順</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>パンダ</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>大車輪</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>四連刻</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>八連荘</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
        <tr>
          <th>人和</th>
          <td><input type="radio" name="kuitan" value="">あり</td>
          <td><input type="radio" name="kuitan" value="">なし</td>
        </tr>
      </tbody>
    </table>
  </div>
  <p>
    <input class="user_name" type="text" name="" value="">ユーザ名/
    <input class="user_id" type="text" name="" value="">ツイッターID
  </p>
  <p><button class="user_add" type="button" name="button">追加</button></p>
  <p><button class="user_add" type="button" name="button">登録</button></p>

  <!-- このボタン、PHP経由で表示すべきか。ページをロードして表示するならその方がよさそう -->
  <button type="button" name="button">ルール登録へ</button>

  <input type="text" name="" value="">
</form>

@include('share.htmlFooter)

</html>
